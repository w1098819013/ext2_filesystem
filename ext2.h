...
#define FILE_CHAR 'f'
#define SYMBOLIC_CHAR  's'

int RoundToTheNearestFour(int size);
struct ext2_dir_entry* createNewDirectoryEntry(char *hardLinkName, int sourceInodeNum, unsigned char file_type);
...
/*
* Takes in a path(char *) and finds the correct inode offset corresponding
* to the path, ENOENT if the path is invalid.
*/
int findInodeOffsetFromPath(char *path);

/*
* Returns the inode table on disk by using pointer arithmatic 
* to get to the "offset" block on the disk.
*/
void* getBlock(unsigned int offset);

/*
* Returns the inode table on disk
*/
struct ext2_inode* getInodeTable();

/*
* Gets the file type that an inode is pointing to using it's i_mode member.
*/
char getFileModeChar(unsigned short imode);

/*
* Gets the file type from the directory entry.
*/
char getFileType(unsigned char filetype);

/*
* Gets the inode using the corresponding inode offset.
*/
